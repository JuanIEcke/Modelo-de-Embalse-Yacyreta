...
# --- 0. CONFIGURACIÓN INICIAL Y CSS PERSONALIZADO (IMAGEN ELIMINADA) ---
# Se eliminó toda la lógica de carga de imagen y el CSS de background-image.
# Solo dejamos un CSS básico para el título y el sidebar para mantener el estilo deseado
custom_css = f"""
<style>
...
.st-emotion-cache-18ni7ap {{ /* Ajuste del margen superior para el contenido principal */
    margin-top: -1cm; /* Se redujo el margen ya que no hay imagen de fondo que lo justifique */
}}
...
with st.sidebar.expander("2. Parámetros Principales de Simulación", expanded=True):
    potencia_objetivo_mw = st.number_input(
        "Potencia Objetivo [MW]",
        min_value=100.0,
        max_value=5000.0,
        value=3100.0,
        step=50.0,
        help="Potencia objetivo que el sistema intentará generar."
    )
    modo_verificacion = st.selectbox(
        "Modo de Verificación",
        options=["Desactivado", "Activado"],
        index=0,
        help="'Activado' usa el caudal medio de la serie para verificar el balance de masa."
    )
with st.sidebar.expander("3. Parámetros del Embalse", expanded=True):
    ...
    if use_avg_level:
        ...
        nivel_embalse_inicial = st.number_input(
            "Nivel Inicial (manual) [m]", 
            value=82.5, 
            min_value=nivel_minimo_embalse_m, 
            max_value=nivel_maximo_operativo_m
        )
with st.sidebar.expander("4. Configuración de Turbinas", expanded=False):
    ...
    cant_turbinas_nuevas = st.number_input("Cantidad de turbinas 'nuevas'", min_value=0, max_value=20, value=3, step=1)
...
if st.sidebar.button("▶️ Ejecutar Simulación", type="primary"):
    if uploaded_file is None:
        st.error("❌ Por favor, carga el archivo de datos 'Datos.xlsx' antes de ejecutar la simulación.")
    elif not os.path.exists("modelo_backend.py"):
        st.error("❌ No se encontró el archivo 'modelo_backend.py'. Asegúrate de que esté en la misma carpeta que 'frontend.py'.")
    else:
